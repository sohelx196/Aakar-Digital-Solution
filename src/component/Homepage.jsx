...
<section className="bg-slate-900 py-16 px-4 sm:px-10 lg:px-20 overflow-hidden"> {/* added overflow-hidden */}
  ...
  <div
    ref={scrollRef}
    className="mt-16 flex gap-6 overflow-x-auto snap-x snap-mandatory scroll-smooth no-scrollbar"
    style={{ scrollBehavior: "smooth" }}
  >
    {products.map((product, index) => (
      <div key={index} className="flex-shrink-0 w-full sm:w-[400px] flex justify-center snap-start">
        <DarkVariantExample product={product.img} alt={product.title} />
      </div>
    ))}
  </div>
  ...
  <div className="text-center mt-10">
    <Link to="aakarDigitalProduct">
      <button className="text-xs sm:text-sm md:text-base lg:text-lg font-quicksand hover:border border-white-700 border-b-4 text-white font-semibold px-4 py-2 rounded-xl border hover:shadow-md hover:scale-105 transition-all duration-100">
        See More
      </button>
    </Link>
  </div>
